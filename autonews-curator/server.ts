...
import { CURATION_SYSTEM_PROMPT } from "./src/lib/constants";
...
// Helper to escape literal newlines and control characters inside double-quoted string values in malformed JSON
...
// Robust JSON parsing, cleaning, and repairing helper
function safeParseJSON(text: string): any {
  ...
  // 2. Extract only the substring between the first and last '{' or '[' if there is surrounding conversational text
  ...
  // FIRST TRY: Try standard parsing right away on the raw extracted block
  try {
    return JSON.parse(cleaned);
  } catch (_e) {
    // Attempt repairs silently if raw JSON.parse fails
  }

  // REPAIR STEP A: Escape literal newlines and control characters within double-quoted strings
  ...
  // REPAIR STEP B: Line-by-line quote and escape repair for single key-value properties
  ...
  // REPAIR STEP C: Repair missing commas between objects/arrays and missing commas between properties
  ...
  // REPAIR STEP D: Remove invalid control characters
  try {
  ...
  // ADVANCED BULLETPROOF REPAIR PATHWAY (SEGMENT PARSER):
  try {
...
// Inner helper for parsing single flat malformed objects
function parseMalformedJSONObject(objStr: string): any {
  ...
  // Find all keys and their positions in the string
  ...
  // Sort key positions by their start index in the string
  ...
  for (let i = 0; i < keyPositions.length; i++) {
    ...
    // Extract the raw value substring
    let rawVal = objStr.slice(current.valueStart, nextStart).trim();
    
    // Clean trailing comma, closing brace, and whitespace
    ...
    if (rawVal.startsWith('"')) {
      // It's a string. Let's find the outer bounds.
...
// Initialize Gemini SDK on the server-side with user-agent for telemetry
...
function getRetryDelay(err: any): number {
  try {
    ...
    // 1. Try to find "retryDelay":"38s" in JSON
    ...
    // 2. Try to find "retry in 38.281044093s"
...
// Model cascade priority list for high-quota reliability (using strictly supported flash models)
const MODEL_CASCADE = [
  "gemini-2.5-flash",
  "gemini-2.0-flash",
  "gemini-1.5-flash"
...
// Helper to call generateContent with retry and model fallback cascade
async function generateContentWithRetry(
  ...
  // Construct model queue starting with startModel, followed by other fallback candidates
  ...
  while (attempt < maxAttempts) {
    ...
    // Remove thinkingConfig dynamically if the active model doesn't support reasoning (thinking)
    ...
    try {
      ...
      // On 429, 503, or 404, transition to next flash model with an appropriate wait
...
async function startServer() {
  ...
  // Set up robust limits to accept complex OCR/Text payloads or image Base64 strings safely
  app.use(express.json({ limit: "50mb" }));
  app.use(express.urlencoded({ limit: "50mb", extended: true }));

  // API Route: Curate News
...
4. Return a JSON object containing the array of AT LEAST 10 to 15 shortlisted items.`
      ...
      // If fewer than 10 items were extracted, perform targeted second-pass accumulation
...
Return a JSON object with the additional extracted items.`;
      ...
      // Re-calculate summary counts dynamically
  ...
  // Helper to parse combined raw text with === FILE: ... === markers
  ...
  // API Route: Shortlist Whole Week News
  app.post("/api/shortlist-week", async (req, res, next) => {
    try {
      ...
      // Parse individual files from combinedText
...
You MUST perform an exhaustive re-scan of "${file.name}" to extract AT LEAST 7 TO 12 MORE distinct automotive news items across all 7 categories. Check both Left and Right columns of all tables and every section header! Do NOT repeat existing items.`;
      ...
      // Ensure strict SOP cleaning for all compiled items
...
// Helper function for smart local text extraction fallback when API limits are hit
function extractLocalDetailsForChunk(chunk: any[], rawText: string, allItemsList: any[] = []) {
  ...
  return chunk.map((item: any) => {
    ...
    // Strategy 1: Exact substring match (case insensitive) skipping TOC lines
    ...
    // Strategy 2: First 4 significant tokens within 120 chars on adjacent lines
    if (matchedPos === -1) {
    ...
    // Strategy 3: First 2-3 significant tokens within 80 chars
    if (matchedPos === -1) {
    ...
    // Strategy 4: Line matching against significant tokens
    if (matchedPos === -1) {
    ...
    if (matchedPos !== -1) {
      // Body text starts AFTER the headline line
      ...
      // Find boundary of NEXT article headline
      ...
      // Clean out website widgets / recommended article lines
  ...
  // API Route: Extract Detailed News for Word Docs
...
`;
        ...
        try {
          ...
          if (parsed && Array.isArray(parsed)) {
            // Apply strict SOP cleaning to full text and headlines on the compiled array
  ...
  // Robust JSON error handling middleware for all API routes (prevents returning HTML for body-parser or route errors)
  ...
  // Serve static dist in production, use Vite dev middleware in development
